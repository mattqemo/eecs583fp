...
namespace Morg {
  ...
  private:
    void dump() {
      errs() << "-----" << L->getLoopPreheader()->getName() << "-----\n";
      for (auto& inst : L->getLoopPreheader()->getInstList()) {
        errs() << inst << "\n";
      }
      errs() << "\n\n";
      for (auto* bb : L->getBlocks()) {
        errs() << "-----" << bb->getName() << "-----\n";
        for (auto& inst : bb->getInstList()) errs() << inst << "\n";
        errs() << "\n\n";
      }
    }
  };
  bool DoLICM(Loop *L, const std::unordered_set<BasicBlock*>& hotPathSet, BranchProbabilityInfo *bpi, DominatorTree *dt, BlockFrequencyInfo *bfi, DominanceFrontier* df, bool performance) {
    using llvm::LoadInst;
    using llvm::StoreInst;
    using llvm::AllocaInst;
    // rm dot/hw2correct1.cfg.pdf && ./viz.sh hw2correct1 && cd HW2 && make -j4 && cd - && ./run.sh hw2correct1 2> out.txt
    auto dump = DumpLoopIR(L);
    // Maps from an almost-invariant pointer to the loads from that pointer
    std::unordered_map<AllocaInst*, std::unordered_set<LoadInst*>> addrsToLoads;
    // Maps from an almost-invariant pointer to the stores to that pointer
    std::unordered_map<AllocaInst*, std::unordered_set<StoreInst*>> addrsToStores;
    bool changed = false;

    auto isAlmostInvariant = [&](AllocaInst* addrPtr) {
      if (addrsToLoads.count(addrPtr) || addrsToStores.count(addrPtr)) return true;
      bool storedInColdPath = false;
      for (auto* user : addrPtr->users()) {
        if (auto* storeInst = dyn_cast<StoreInst>(user)) {
          if (hotPathSet.count(storeInst->getParent())) return false;
          storedInColdPath = storedInColdPath || L->contains(storeInst->getParent());
        }
      }
      return storedInColdPath; // don't consider fully invariant memory
    };

    // Find all almost invariant alloca-ed addresses
    for (auto* bb : hotPathSet) {
      for (auto& inst : *bb) {
        if (auto* loadInst = dyn_cast<LoadInst>(&inst)) {
          errs() << "load: " << *loadInst << "\n";
          auto* ptr = dyn_cast<AllocaInst>(loadInst->getPointerOperand());
          if (ptr && isAlmostInvariant(ptr)) {
            errs() << "almost invar!\n";
            addrsToLoads[ptr].insert(loadInst);
          }
        }
      }
    }

    // Add all stores to almost-invariant addresses
    for (auto& addrAndLoads : addrsToLoads) {
      for (auto* user : addrAndLoads.first->users()) {
        if (auto* storeInst = dyn_cast<StoreInst>(user)) {
          if (L->contains(storeInst)) addrsToStores[addrAndLoads.first].insert(storeInst);
        }
      }
    }

    auto BBExecCount = [&](const BasicBlock* bb) { return bfi->getBlockProfileCount(bb).getValueOr(0); };
    constexpr unsigned int LOAD_COST = 5;
    constexpr unsigned int STORE_COST = 5;
    constexpr unsigned int PHI_COST = 5;
    for (auto addrAndLoadsIt = addrsToLoads.begin(); addrAndLoadsIt != addrsToLoads.end();/*auto& addrAndLoads : addrsToLoads*/) {
      int64_t instrAddedByHoist = 0;
      for (auto* loadInst : addrAndLoadsIt->second) {
        instrAddedByHoist -= LOAD_COST * BBExecCount(loadInst->getParent()); // load comes from its current parent
        errs() << instrAddedByHoist << " load\n";
        instrAddedByHoist += LOAD_COST * BBExecCount(L->getLoopPreheader()); // Load goes to preheader
        errs() << instrAddedByHoist << " loadprehead\n";
      }
      std::unordered_set<BasicBlock*> phiNodeInsertPoints; // Track duplicate phi nodes within this set of dependent stores
      for (auto* storeInst : addrsToStores[addrAndLoadsIt->first]) {
        for (auto* frontierBB : df->find(storeInst->getParent())->second) {
          if (phiNodeInsertPoints.insert(frontierBB).second) {
            instrAddedByHoist += PHI_COST * BBExecCount(frontierBB); // Add phi to dom frontier of this store
            errs() << instrAddedByHoist << " phi\n";
          }
        }
        instrAddedByHoist -= STORE_COST * BBExecCount(storeInst->getParent()); // Store will be deleted
        errs() << instrAddedByHoist << " store\n";
      }
      errs() << "Predicting " << instrAddedByHoist << " instructions will be added by hoisting loads to " <<
        *addrAndLoadsIt->first << ", so " << (instrAddedByHoist > 0 ? "not" : "") << " hoisting it\n";
      if (instrAddedByHoist > 0) {
        addrsToStores.erase(addrAndLoadsIt->first); // Don't hoist this!
        addrAndLoadsIt = addrsToLoads.erase(addrAndLoadsIt);
      } else {
        ++addrAndLoadsIt;
      }
    }

    // Execute hoist of loads
    for (auto& addrAndLoads : addrsToLoads) {
      for (auto* loadInst : addrAndLoads.second) {
        changed = true;
        errs() << "Hoisting " << *loadInst << "\n";
        loadInst->moveBefore(L->getLoopPreheader()->getTerminator());
      }
    }

    // Indicate that all stores create a new available value for the load
    for (auto& addrAndLoads : addrsToLoads) {
      AllocaInst* addr = addrAndLoads.first;
      SSAUpdater updater;
      llvm::SmallVector<char, 16> buf;
      updater.Initialize(addr->getAllocatedType(), llvm::Twine(addr->getName(), "hoist").toStringRef(buf));
      auto lastLoadIt = std::find_if(L->getLoopPreheader()->rbegin(), L->getLoopPreheader()->rend(), [&](auto& inst) {
        return addrAndLoads.second.count(dyn_cast<LoadInst>(&inst));
      });
      assert(lastLoadIt != L->getLoopPreheader()->rend() && "hoisted load claimed to exist, but not found");
      errs() << "Adding value with type " << *lastLoadIt->getType() << "\n";
      updater.AddAvailableValue(L->getLoopPreheader(), &*lastLoadIt);
      for (auto* store : addrsToStores[addr]) {
        // Find rearmost store in this store's basic block - that value is available
        Instruction* temp = &store->getParent()->getInstList().back();
        while (temp != nullptr && !addrsToStores[addr].count(dyn_cast<StoreInst>(temp))) temp = temp->getPrevNode();
        assert(temp != nullptr && "store claimed to exist, but not found");
        errs() << "Adding value with type " << dyn_cast<StoreInst>(temp)->getValueOperand()->getType() << "\n";
        updater.AddAvailableValue(store->getParent(), dyn_cast<StoreInst>(temp)->getValueOperand());
      }
      for (auto* load : addrAndLoads.second) {
        auto use_it = load->use_begin(), use_end = load->use_end();
        while (use_it != use_end) updater.RewriteUseAfterInsertions(*use_it++);
      }
    }

    if (!performance) return changed;

    std::unordered_set<Instruction*> usedByRemoved;
    auto destroy = [&](Instruction* inst) {
      for (auto& use : inst->operands()) {
        if (auto* usedInst = dyn_cast<Instruction>(use.get())) usedByRemoved.insert(usedInst);
      }
      inst->eraseFromParent();
    };

    for (auto& addrAndStores : addrsToStores) {
      for (auto* store : addrAndStores.second) {
        errs() << "STORE " << store << ": " << *store << "\n";
        // In this basic block, whenever this stored value is used (loaded), redirect that use to the source of the store
        // Stop search when a new store to the same location is found
        for (auto* inst = store->getNextNode(); inst != nullptr && !addrAndStores.second.count(dyn_cast<StoreInst>(inst));) {
          errs() << *inst << " -- " << inst->getNextNode() << "\n";
          auto* next = inst->getNextNode();
          if (auto* loadInst = dyn_cast<LoadInst>(inst)) {
            if (loadInst->getPointerOperand() == store->getPointerOperand()) {
              loadInst->replaceAllUsesWith(store->getValueOperand());
              destroy(loadInst);
            }
          }
          inst = next;
    ...
    while (true) {
      std::unordered_set<Instruction*> toHoist;
      for (auto* bb : L->getBlocks()) {
        for (auto& inst : *bb) {
          if (inst.getOpcode() != Instruction::PHI && inst.isSafeToRemove() && !inst.mayReadOrWriteMemory() && llvm::all_of(inst.operand_values(), [=](auto* val) { return L->isLoopInvariant(val); })) {
            toHoist.insert(&inst);
          }
        }
      }
      if (toHoist.empty()) break; // TODO this is gross
    ...
    // If any optimizations created dead code, eliminate
    for (auto* inst : usedByRemoved) {
      if (inst->hasNUses(0)) inst->eraseFromParent();
    }
    std::vector<Instruction*> toRemove;
    for (auto* bb : L->getBlocks()) {
      for (auto& inst : *bb) {
        if (inst.isSafeToRemove() && !inst.mayWriteToMemory() && inst.hasNUses(0)) {
          toRemove.push_back(&inst);
        }
      }
    }
    std::for_each(toRemove.begin(), toRemove.end(), [](auto* inst) { inst->eraseFromParent(); });

    return changed;
  }

  bool FPLICM(Loop *L, BranchProbabilityInfo *bpi, DominatorTree *dt, BlockFrequencyInfo *bfi, DominanceFrontier* df, bool performance) {
    bool changed = false;
    const auto BRANCH_PROB_THRESHOLD = BranchProbability(4, 5);
    BasicBlock* hotPathTail = L->getHeader();
    std::unordered_set<BasicBlock*> hotPathSet = {hotPathTail};
    while (true) {
        BasicBlock* tempHotPathTail = hotPathTail;
        for (auto* succ : successors(hotPathTail->getTerminator())) {
          if (bpi->getEdgeProbability(hotPathTail, succ) >= BRANCH_PROB_THRESHOLD) { // likely edge
            if (hotPathSet.count(succ)) { // backedge or weird inner cycle
              return dt->dominates(succ, hotPathTail) ? DoLICM(L, hotPathSet, bpi, dt, bfi, df, performance) : false;
            }
            if (!L->contains(succ)) return false; // No frequent loop path
            hotPathTail = succ;
            hotPathSet.insert(succ);
            break;
          }
        }
        if (hotPathTail == tempHotPathTail) return false; // If no next frequent edge found, no frequent path
    }
    return changed;
  }
} // namespace Morg
