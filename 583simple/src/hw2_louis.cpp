...
//===----------------------------------------------------------------------===//
//
// EECS583 F20 - This pass can be used as a template for your Frequent Path LICM
...
static void FPhoist(Instruction &I, const DominatorTree *DT, const Loop *CurLoop,
                  BasicBlock *Dest, ICFLoopSafetyInfo *SafetyInfo,
                  MemorySSAUpdater *MSSAU, ScalarEvolution *SE,
                  OptimizationRemarkEmitter *ORE);
static bool FPsink(Instruction &I, LoopInfo *LI, DominatorTree *DT,
...
static bool FPhoistRegion(DomTreeNode *N, AliasAnalysis *AA, LoopInfo *LI,
...
static bool FPsinkRegion(DomTreeNode *N, AliasAnalysis *AA, LoopInfo *LI,
...
static bool FPcanSinkOrHoistInst(Instruction &I, AAResults *AA, DominatorTree *DT,
...
static Instruction* getFirstFunctionInstFromLoop(Loop *L);

static StoreInst* castUserToStoreInst(User* U);
...
struct LoopInvariantCodeMotion {
  ...
  bool performFPLICM(Loop *L, AliasAnalysis *AA, LoopInfo *LI, DominatorTree *DT,
                 TargetLibraryInfo *TLI, TargetTransformInfo *TTI,
                 ScalarEvolution *SE, MemorySSA *MSSA,
                 OptimizationRemarkEmitter *ORE, bool DeleteAST,
                 BranchProbabilityInfo *BPI, BlockFrequencyInfo *BFI);
...
private:
  ...
  unsigned LicmMssaNoAccForPromotionCap;
  std::unordered_map<Value*, std::pair<AllocaInst*, LoadInst*>> RefsForLIPtr;

  std::unique_ptr<AliasSetTracker>
  collectAliasInfoForLoop(Loop *L, LoopInfo *LI, AliasAnalysis *AA);

  bool handleFPLoad(Loop *L, const std::unordered_set<BasicBlock*>& frequentPath, LoadInst* LI);
  std::unordered_set<BasicBlock*> getLoopMostFrequentPath(Loop *L, BranchProbabilityInfo *BPI);
  std::vector<LoadInst*> getFPLoads(const std::unordered_set<BasicBlock*>& frequentPath);
};
...
struct FPLICMPass : public LoopPass {
  ...
  FPLICMPass(
      ...
      : LoopPass(ID), FPLICM(LicmMssaOptCap, LicmMssaNoAccForPromotionCap) {
  }
  ...
  bool doFinalization() override {
    auto &AliasSetMap = FPLICM.getLoopToAliasSetMap();
...
private:
  ...
  std::vector<LoadInst*> constFPLoads; /* need vector because order of traversal matters */
  std::unordered_map<Value*, std::pair<AllocaInst*, LoadInst*>> RefsForHoistedInst;

  std::unique_ptr<AliasSetTracker>
  collectAliasInfoForLoop(Loop *L, LoopInfo *LI, AliasAnalysis *AA);

  bool handleFPLoad(Loop *L, LoopInfo* LoopInf, const std::unordered_set<BasicBlock*>& FP_set, const std::vector<BasicBlock*>& FP_vec, LoadInst* LI);
  std::vector<LoadInst*> getConstFPLoads(const std::vector<LoadInst*>& FPLoads, const std::unordered_set<BasicBlock*>& FP);
  std::vector<StoreInst*> getLoopWriterSIs(Value* LIptr, bool& hasFrequentStore, Loop* L);
  void hoistInstruction(Instruction* I, Loop* L, std::unordered_map<Instruction*, Instruction*>& HoistedInstToPreheaderVal);
  void fixUpInfrequentStore(StoreInst* SI, LoadInst* LI, Loop* L, const std::vector<Instruction*>& FPConstInst);
  std::vector<BasicBlock*> getLoopMostFrequentPath(
    Loop *L, BranchProbabilityInfo *BPI, const BranchProbability cumulMinThreshold
  );
  std::vector<LoadInst*> getFPLoads(const std::vector<BasicBlock*>& frequentPath);
};
...
bool Correctness::LoopInvariantCodeMotion::runOnLoop(
  ...
  if (!MSSA) {
    ...
    LLVM_DEBUG(dbgs() << "FPLICM: Using MemorySSA.\n");
  ...
  if (Preheader) {
    Changed |= FPhoistRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI, L,
            CurAST.get(), MSSAU.get(), SE, &SafetyInfo, Flags, ORE);
    Changed |= performFPLICM(L, AA, LI, DT, TLI, TTI, SE, MSSA, ORE, DeleteAST, BPI, BFI);
  }

  return Changed;
}

bool Correctness::LoopInvariantCodeMotion::performFPLICM(Loop *L, AliasAnalysis *AA, LoopInfo *LI, DominatorTree *DT,
                 TargetLibraryInfo *TLI, TargetTransformInfo *TTI,
                 ScalarEvolution *SE, MemorySSA *MSSA,
                 OptimizationRemarkEmitter *ORE, bool DeleteAST,
                 BranchProbabilityInfo *BPI, BlockFrequencyInfo *BFI) {

  std::unordered_set<BasicBlock*> FP = getLoopMostFrequentPath(L, BPI);
  std::vector<LoadInst*> FPLoads = getFPLoads(FP);

  bool Changed = false;
  for (LoadInst* LI : FPLoads) {
    Changed |= handleFPLoad(L, FP, LI);
  }

  return Changed;
}

bool Correctness::LoopInvariantCodeMotion::handleFPLoad(
  Loop *L, const std::unordered_set<BasicBlock*>& FP, LoadInst* LI
) {
  Value* LIptr = LI->getPointerOperand();

  if (!isa<AllocaInst>(LIptr)) {
    return false; // non constant address
  }

  std::vector<StoreInst*> writerSIs;
  for (User* U : LIptr->users()) {
    StoreInst* SI = castUserToStoreInst(U);
    if (SI && SI->getPointerOperand() == LIptr) {
      if (FP.count(SI->getParent())) {
        return false; // can't hoist if there is related store in FP
      }
      else {
        writerSIs.push_back(SI);
      }
    }
  }
  ...
  Type* Ty = LI->getType();
  AllocaInst* NewVar;
  LoadInst* HeaderLoadVar;

  if (RefsForLIPtr.count(LIptr) == 0) {
    /* Function Entry Block */
    NewVar = new AllocaInst(Ty, 0, nullptr, "", getFirstFunctionInstFromLoop(L));

    /* Loop Preheader */
    LoadInst* InitVal = new LoadInst(LIptr, "", L->getLoopPreheader()->getTerminator());
    StoreInst* StoreToVar = new StoreInst(InitVal, NewVar, L->getLoopPreheader()->getTerminator());

    /* Loop Header */
    HeaderLoadVar = new LoadInst(NewVar, "", LI);
    RefsForLIPtr[LIptr] = {NewVar, HeaderLoadVar};
  }
  else {
    NewVar = RefsForLIPtr[LIptr].first;
    HeaderLoadVar = RefsForLIPtr[LIptr].second;
  }

  /* Handle Infrequent Store: insert store after for NewVar */
  for (StoreInst* SI : writerSIs) {
    StoreInst* FixUpStore = new StoreInst(SI->getValueOperand(), NewVar, SI);
  }

  /* Handle reader-dependent instructions */
  LI->replaceAllUsesWith(HeaderLoadVar);
  LI->eraseFromParent();

  return true;
}

std::unordered_set<BasicBlock*> Correctness::LoopInvariantCodeMotion::getLoopMostFrequentPath(
  Loop *L, BranchProbabilityInfo *BPI
) {

  std::unordered_set<BasicBlock*> frequentPath;
  BasicBlock* block = L->getBlocksVector().front();
  BranchProbability cumulProb(1, 1), absMinThreshold(4, 5);

  do {
    frequentPath.insert(block);

    BasicBlock* mostProbSucc = nullptr;
    for (BasicBlock* succ : successors(block)) {
      const BranchProbability& edgeProba = BPI->getEdgeProbability(block, succ);

      if (edgeProba >= absMinThreshold) {
        mostProbSucc = succ;
        break;
      }
    }
    // maybe add check to see if all blocks in loop (but I think benchmarks assume that should be fine)
    if (mostProbSucc == nullptr) return {};
    else block = mostProbSucc;
  } while (block != L->getHeader());

  return frequentPath;
}

std::vector<LoadInst*> Correctness::LoopInvariantCodeMotion::getFPLoads(
  const std::unordered_set<BasicBlock*>& frequentPath
) {
  std::vector<LoadInst*> FPLoads;
  for (BasicBlock* BB : frequentPath) {
    for (Instruction& I : *BB) {
      if (LoadInst* LI = dyn_cast<LoadInst>(&I)) {
        FPLoads.push_back(LI);
      }
    }
  }
...
bool Performance::LoopInvariantCodeMotion::runOnLoop(
  ...
  // *****************************************************************
  if (LI->getLoopDepth(L->getHeader()) == 1) {
    Changed |= FPhoistRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI, L,
            CurAST.get(), MSSAU.get(), SE, &SafetyInfo, Flags, ORE);
    Changed |= performFPLICM(L, AA, LI, DT, TLI, TTI, SE, MSSA, ORE, DeleteAST, BPI, BFI);
  }
...
bool Performance::LoopInvariantCodeMotion::performFPLICM(Loop *L, AliasAnalysis *AA, LoopInfo *LI, DominatorTree *DT,
                 TargetLibraryInfo *TLI, TargetTransformInfo *TTI,
                 ScalarEvolution *SE, MemorySSA *MSSA,
                 OptimizationRemarkEmitter *ORE, bool DeleteAST,
                 BranchProbabilityInfo *BPI, BlockFrequencyInfo *BFI)
{
  BranchProbability cumulMinThreshold = BranchProbability(1, 3); // 33%
  std::vector<BasicBlock*> FP_vec = getLoopMostFrequentPath(L, BPI, cumulMinThreshold);
  std::unordered_set<BasicBlock*> FP_set(FP_vec.begin(), FP_vec.end());

  std::vector<LoadInst*> FPLoads = getFPLoads(FP_vec);
  constFPLoads = getConstFPLoads(FPLoads, FP_set);

  bool Changed = false;
  for (LoadInst* LoadI : constFPLoads) {
    Changed |= handleFPLoad(L, LI, FP_set, FP_vec, LoadI);
  }
  return Changed;
}


bool Performance::LoopInvariantCodeMotion::handleFPLoad(
  Loop *L, LoopInfo* LoopInf, const std::unordered_set<BasicBlock*>& FP_set,
  const std::vector<BasicBlock*>& FP_vec, LoadInst* LI
) {
  Value* LIptr = LI->getPointerOperand();

  if (!isa<AllocaInst>(LIptr)) {
    return false; // non constant address, TODO! handle later
  }

  bool hasFrequentStore = false;
  std::vector<StoreInst*> writerSIs = getLoopWriterSIs(LIptr, hasFrequentStore, L);
  if (hasFrequentStore) {
    return false;
  }

  std::vector<Instruction*> FPConstInst; // instructions that become const in FP
  for (User* U : LI->users()) {
    /* NOTE: no enough, probably want a queue to iterate or smth like taht*/
    if (Instruction* I = dyn_cast<Instruction>(U)) {
      if (dyn_cast<PHINode>(I) || FP_set.count(I->getParent()) == 0 || I == LI){
        errs() << "Not invariant: \t" << *I << '\n';
        continue; // PHI node implies value changes over iterations, and want Instruction in FP
      }

      if (dyn_cast<LoadInst>(I)) {
        continue; // ignore other loads for now
      }
      ...
      for (Value* V : I->operand_values()) {
        ...
        else if (Instruction* I_op = dyn_cast<Instruction>(V)) {
          LoadInst* LI_op = dyn_cast<LoadInst>(I_op);
          if (LI_op && std::count(constFPLoads.begin(), constFPLoads.end(), LI_op) // operand is a load we will hoist
              || std::count(FPConstInst.begin(), FPConstInst.end(), I_op)) { // part of an already marked as constant instruction
            continue;
          }
          else {
            errs() << "Non constant: " << *I << '\t' << *I_op << '\n';
            constant = false;
            break;
          }
        }
  ...
  std::unordered_map<Instruction*, Instruction*> HoistedInstToPreheaderVal;
  hoistInstruction(LI, L, HoistedInstToPreheaderVal);
  for (Instruction* I : FPConstInst) {
    hoistInstruction(I, L, HoistedInstToPreheaderVal);
  }

  for (StoreInst* SI : writerSIs) {
    fixUpInfrequentStore(SI, LI, L, FPConstInst);
  }
  LI->eraseFromParent();
  for (Instruction* I : FPConstInst) {
    I->eraseFromParent();
...
// Does NOT handle store fix up OR clean up
/* !!!!!!!!!!!!!!!!!!!!!!
TODO! fix for double load on same address
TODO! do not allocate a variable for a value if it's only used as a temp
!!!!!!!!!!!!!!!!!!!!!!!!!!*/
void Performance::LoopInvariantCodeMotion::hoistInstruction(
  Instruction* I, Loop* L, std::unordered_map<Instruction*, Instruction*>& HoistedInstToPreheaderVal
) {
  LoadInst* LI = dyn_cast<LoadInst>(I);

  LoadInst* HeaderLoadVar;
  if (!LI || RefsForHoistedInst.count(I) == 0) {
    /* Function Entry Block */
    AllocaInst* NewVar = new AllocaInst(I->getType(), 0, nullptr, "", getFirstFunctionInstFromLoop(L));

    /* Loop Preheader */
    Instruction* InitInst = I->clone();
    // Fix Operands
    for (int i = 0; i < InitInst->getNumOperands(); ++i) {
      Value* V = InitInst->getOperand(i);
      if (Instruction* I_op = dyn_cast<Instruction>(V)) {
        auto it_op = HoistedInstToPreheaderVal.find(I_op);
        if (it_op != HoistedInstToPreheaderVal.end()) { // checks if operand is defined in loop (if so, must have been hoisted by our logic)
          InitInst->setOperand(i, it_op->second);
        }
      }
    }

    InitInst->insertBefore(L->getLoopPreheader()->getTerminator());
    errs() << "Init Inst: " << *InitInst << '\n';
    StoreInst* StoreInitVal = new StoreInst(InitInst, NewVar, L->getLoopPreheader()->getTerminator());

    /* Loop Header */
    HeaderLoadVar = new LoadInst(NewVar, "", I);
    RefsForHoistedInst[I] = {NewVar, HeaderLoadVar};
    HoistedInstToPreheaderVal[HeaderLoadVar] = InitInst; // use that since we replace all use of I below
  }
  else {
    HeaderLoadVar = RefsForHoistedInst[I].second;
  }

  I->replaceAllUsesWith(HeaderLoadVar);
}

void Performance::LoopInvariantCodeMotion::fixUpInfrequentStore(
  StoreInst* SI, LoadInst* LI, Loop* L, const std::vector<Instruction*>& FPConstInst
) {
  std::unordered_map<Instruction*, Value*> OriginalToFixupVal;
  new StoreInst(SI->getValueOperand(), RefsForHoistedInst[LI].first, SI->getParent()->getTerminator());
  OriginalToFixupVal[RefsForHoistedInst[LI].second] = SI->getValueOperand();

  for (Instruction* I : FPConstInst) {
    Instruction* I_clone = I->clone();
    errs() << "Cloning: " << *I << '\n';
    for (int i = 0; i < I_clone->getNumOperands(); ++i) {
      Value* V = I_clone->getOperand(i);
      errs() << "Val: " << *V << '\n';
      if (L->isLoopInvariant(V)) {
        errs() << "Inviarant\n";
        continue;
      ...
      auto it_op = OriginalToFixupVal.find(dyn_cast<Instruction>(V));
      assert(it_op != OriginalToFixupVal.end());
      errs() << "Before assign: " << *(it_op->second) << '\n';
      I_clone->setOperand(i, it_op->second);
      errs() << "After assign\n";
    }

    I_clone->insertBefore(SI->getParent()->getTerminator());
    errs() << "Fix Up\n\tStore: "  << *SI << "\n\tFixUp: " << *I_clone << '\n';
    OriginalToFixupVal[RefsForHoistedInst[I].second] = I_clone;
    new StoreInst(I_clone, RefsForHoistedInst[I].first, SI->getParent()->getTerminator());
  }
}

std::vector<LoadInst*> Performance::LoopInvariantCodeMotion::getConstFPLoads(
  const std::vector<LoadInst*>& FPLoads, const std::unordered_set<BasicBlock*>& FP
) {
  std::vector<LoadInst*> constFPLoads;
  for (LoadInst* LI : FPLoads) {
    Value* LIptr = LI->getPointerOperand();
    if (!isa<AllocaInst>(LIptr)) {
      continue; // non constant address, TODO! handle later
    }
    ...
    if (constant) constFPLoads.push_back(LI);
  }
  return constFPLoads;
}

std::vector<StoreInst*> Performance::LoopInvariantCodeMotion::getLoopWriterSIs(
  Value* LIptr, bool& hasFrequentStore, Loop* L
) {
  std::vector<StoreInst*> writerSIs;
  for (User* U : LIptr->users()) { // non need to check for store along FP because already handled
    if (StoreInst* SI = castUserToStoreInst(U)) {
      if (std::count(L->block_begin(), L->block_end(), SI->getParent())) {
        errs() << "WriteSI: " << *SI << '\n';
        writerSIs.push_back(SI); // only concerned about writers inside Loop
      }
    }
  }
  return writerSIs;
...
std::vector<LoadInst*> Performance::LoopInvariantCodeMotion::getFPLoads(
  const std::vector<BasicBlock*>& frequentPath
) {
  std::vector<LoadInst*> FPLoads;
  for (BasicBlock* BB : frequentPath) {
    for (Instruction& I : *BB) {
...
std::vector<BasicBlock*> Performance::LoopInvariantCodeMotion::getLoopMostFrequentPath(
  Loop *L, BranchProbabilityInfo *BPI,
  const BranchProbability cumulMinThreshold
) {
  ...
  do {
    frequentPath.push_back(block);

    BasicBlock* mostProbSucc = nullptr;
    for (BasicBlock* succ : successors(block)) {
      const BranchProbability& edgeProba = BPI->getEdgeProbability(block, succ);

      if (edgeProba >= absMinThreshold && cumulProb * edgeProba >= cumulMinThreshold) {
        mostProbSucc = succ;
        cumulProb *= edgeProba;
        break;
      }
  ...
  for (DomTreeNode *DTN : reverse(Worklist)) {
    ...
    for (BasicBlock::iterator II = BB->end(); II != BB->begin();) {
      ...
      if (isNotUsedOrFreeInLoop(I, CurLoop, SafetyInfo, TTI, FreeInLoop) &&
          FPcanSinkOrHoistInst(I, AA, DT, CurLoop, CurAST, MSSAU, true, &Flags,
                             ORE) &&
          !I.mayHaveSideEffects()) {
        if (FPsink(I, LI, DT, CurLoop, SafetyInfo, MSSAU, ORE)) {
          if (!FreeInLoop) {
            ++II;
            eraseInstruction(I, *SafetyInfo, CurAST, MSSAU);
          }
  ...
  for (BasicBlock *BB : Worklist) {
    ...
    for (BasicBlock::iterator II = BB->begin(), E = BB->end(); II != E;) {
      ...
      if (CurLoop->hasLoopInvariantOperands(&I) &&
        ...
        FPhoist(I, DT, CurLoop, CFH.getOrCreateHoistedBlock(BB), SafetyInfo,
              MSSAU, SE, ORE);
      ...
      if (I.getOpcode() == Instruction::FDiv &&
        ...
        FPhoist(*ReciprocalDivisor, DT, CurLoop, CFH.getOrCreateHoistedBlock(BB),
      ...
      if (PHINode *PN = dyn_cast<PHINode>(&I)) {
        if (CFH.canHoistPHI(PN)) {
          ...
          FPhoist(*PN, DT, CurLoop, CFH.getOrCreateHoistedBlock(BB), SafetyInfo,
                MSSAU, SE, ORE);
...
/// loop invariant.
///
static bool promoteLoopAccessesToScalars(
    const SmallSetVector<Value *, 8> &PointerMustAliases,
